...
int TP2::init()
{
    ...
    size_t texcoords_size = m_mesh.texcoord_buffer_size();


	//On recupere l'id de l'attribut position du vertex shader "in vec3 position"
    GLint position_attribute = glGetAttribLocation(m_diffuse_texture_shader, "position");
    GLint normal_attribute = glGetAttribLocation(m_diffuse_texture_shader, "normal");
    GLint texcoord_attribute = glGetAttribLocation(m_diffuse_texture_shader, "texcoord");
    ...
    for (const Material& mat : materials.materials)
    {
        ...
        if (diffuse_texture_index != -1)
        {
            ...
            std::cout << texture_file_path << std::endl;
    ...
    //Parmetrizing the sampler for the textures of the mesh
    glGenSamplers(1, &m_mesh_texture_sampler);
    glBindSampler(m_mesh_texture_sampler);
    glSamplerParameteri(m_mesh_texture_sampler, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glSamplerParameteri(m_mesh_texture_sampler, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glSamplerParameteri(m_mesh_texture_sampler, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
    glSamplerParameteri(m_mesh_texture_sampler, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_BORDER);
...
int TP2::render()
{
    ...
    //On selectionne le vao du mesh
    glBindVertexArray(m_mesh_vao);
	//On draw le robot

    for (TriangleGroup& group : m_mesh_triangle_groups)
    {
        GLuint group_texture_id = m_mesh_textures[group.index];
        glBindSampler(m_mesh_texture_sampler);
        glActiveTexture(GL_TEXTURE_3); //The sampler of
    }
